...
| `/changelog.json` | 逐组件上新/更新时间线（404=构建端未启用，回退读网页版「更新日志」栏目） | ~几KB |
...
| 问最近更新："最近上新/更新了什么"、"这周有什么新组件" | 先拉 `/changelog.json`；404 则抓 `zhiku-market.html` 读「更新日志」栏目。浅目录无逐组件时间戳，别硬答 |
...
`_meta.built` 是**整站**构建时间，不是逐组件更新时间。逐组件的上新/更新日期看「更新日志」（网页 `#changelog` 栏目；机读端点 `/zhiku-api/changelog.json` 上线后优先用它）。
